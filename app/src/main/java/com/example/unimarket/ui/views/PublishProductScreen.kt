...
fun PublishProductScreen(
    ...
    // 1) Collect ViewModel UI events for snackbars
    LaunchedEffect(Unit) {
    ...
    LaunchedEffect(Unit) {
        exploreViewModel.uploadState.collect { state ->
            Log.d("PublishScreen", "uploadState = $state")
            snackbarHost.showSnackbar("uploadState = $state")
        }
    }
    ...
    LaunchedEffect(Unit) {
        try {
            ...
            majors = snapshot.documents.mapNotNull { it.toObject(Major::class.java)?.copy(id = it.id) }
        } catch (e: Exception) {
    ...
    // 3) Load classes when major changes
        ...
        selectedMajor?.let { major ->
            try {
                val snapshot = Firebase.firestore
                    ...
                    .get()
                    .await()
                classes = snapshot.documents.mapNotNull { it.toObject(ClassItem::class.java)?.copy(id = it.id) }
            } catch (e: Exception) {
                FirebaseCrashlytics.getInstance().recordException(e)
    ...
    Scaffold(
        ...
        Column(
            ...
            // Major dropdown
            ExposedDropdownMenuBox(expanded = expandedMajor, onExpandedChange = { expandedMajor = !expandedMajor }) {
                OutlinedTextField(
                    value     = selectedMajor?.name.orEmpty(),
                    onValueChange = {},
                    readOnly  = true,
                    label     = { Text("Major") },
                    trailingIcon = { TrailingIcon(expandedMajor) },
                    modifier  = Modifier.fillMaxWidth().menuAnchor()
                )
                DropdownMenu(expanded = expandedMajor, onDismissRequest = { expandedMajor = false }) {
                    majors.forEach { major ->
                        DropdownMenuItem(text = { Text(major.name) }, onClick = {
                            selectedMajor = major
                            expandedMajor = false
                        })
                    }
            ...
            // Class dropdown
            ExposedDropdownMenuBox(expanded = expandedClass, onExpandedChange = {
                if (selectedMajor != null && classes.isNotEmpty()) expandedClass = !expandedClass
            }) {
                OutlinedTextField(
                    ...
                    readOnly = true,
                    label = { Text("Class") },
                    trailingIcon = { TrailingIcon(expandedClass) },
            ...
            Spacer(Modifier.height(8.dp))
            OutlinedTextField(value = labelsText, onValueChange = { labelsText = it }, label = { Text("Labels (comma separated)") }, modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(8.dp))
            ...
            Button(
                onClick = {
                    ...
                    if (selectedMajor != null && selectedClass != null && title.isNotBlank()
                        && description.isNotBlank() && price != null && imageUri.value != null
                    ) {
                        Log.d("PublishScreen", "CLICK! major=$selectedMajor, class=$selectedClass, title='$title', uri=${imageUri.value}")
                        exploreViewModel.uploadProductImage(imageUri.value!!)
                        // subscriber will see "Image queued for upload" -> "Image uploaded successfully"
                        // publishing happens via callback below once image upload succeeded
                        scope.launch {
                            exploreViewModel.uploadState
                                .filterIsInstance<ImageUploadState.Success>()
                                .first()
                            val downloadUrl = (exploreViewModel.uploadState.value as ImageUploadState.Success).remotePath
                            val product = Product(
                                classId   = selectedClass!!.id,
                                createdAt = Timestamp.now(),
                                description = description,
                                imageUrls   = listOf(downloadUrl),
                                labels      = labelsText.split(",").map(String::trim).filter(String::isNotEmpty),
                            ...
                            exploreViewModel.publishProduct(
                                product,
                                onSuccess = { navController.navigate("explore") { popUpTo("explore") { inclusive = true } } },
                                onFailure = { msg ->
                                    FirebaseCrashlytics.getInstance().recordException(Exception(msg))
                                }
                            )
                        }
                    } else {
                        scope.launch { snackbarHost.showSnackbar("Please fill all fields correctly.") }
                    }
                },
                enabled = !isLoading,
